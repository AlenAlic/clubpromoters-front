<template>
  <v-card>
    <v-card-title>
      {{ $t("club_owner.active_parties.title") }}
      <v-spacer></v-spacer>
      <v-text-field
        v-model="search"
    ...
    >
      ...
      <template v-slot:item.action="{ item }">
        <v-tooltip left>
          ...
          <span>{{ $t("club_owner.active_parties.view") }}</span>
        </v-tooltip>
      </template>
    </v-data-table>
...
export default {
  ...
  data: function() {
    return {
      ...
      headers: [
        {
          text: i18n.t("club_owner.active_parties.table.headers.name"),
          value: "name"
        },
        {
          text: i18n.t("club_owner.active_parties.table.headers.location"),
          value: "location.name"
        },
        {
          text: i18n.t("club_owner.active_parties.table.headers.start_date"),
          value: "start_date"
        },
        {
          text: i18n.t("club_owner.active_parties.table.headers.duration"),
          value: "duration"
        },
        {
          text: i18n.t("club_owner.active_parties.table.headers.commission"),
          value: "commission"
        },
        {
          value: "action",
  ...
  computed: {
    items() {
      return this.$store.state.club_owner_parties.activeParties.map(p =>
        Object.assign(p, {
          duration: `${this.$util
